...
class MonsterCard(AbstractButton):
    ...
    def __init__(self, name, x, y, monster):
        ...
        self.base_x = x
        self.base_y = y
        self.health = PercentBar("player-health", x, y + 8,
                                 simple.draw_rect(160, 8, (50, 255, 50)))
        self.action = PercentBar("player-action", x, y + 20,
                                 simple.draw_rect(160, 8, (50, 100, 50)))
        ...
        if self.monster:
            if self.monster.graphic.get("neutral"):
                try:
                    ...
                    monster_image = simple.draw_image(filename, 3)
                except pygame.error:
        ...
        if self.monster:
            if self.monster.graphic.get("hover"):
                try:
                    ...
                    logging.info("Attempting to create a default hover")
                    try:
                        filename = self.monster.graphic.get("neutral")
                        base_monster_image = simple.draw_image(filename, 1)
                        monster_large = pygame.transform.scale(
                            base_monster_image,
                            (base_monster_image.get_width() + 2,
                             base_monster_image.get_height() + 2))
                        monster_large.fill((255, 255, 0, 0),
                                           special_flags=pygame.BLEND_RGBA_ADD)
                        monster_image = pygame.Surface(
                            base_monster_image.get_size(), pygame.SRCALPHA)
                        monster_image.fill((0, 0, 0, 0))
                        monster_image.blit(monster_large, (-1, -1))
                        monster_image.blit(base_monster_image, (0, 0))
                        monster_image = pygame.transform.scale(
                            monster_image, (monster_image.get_width() * 3,
                                            monster_image.get_height() * 3))
                    except pygame.error:
                        logging.warning(
                            "Attempt failed could not load: " + filename)
                        monster_image = simple.draw_rect(
                            240, 80, (255, 255, 0))
                    else:
